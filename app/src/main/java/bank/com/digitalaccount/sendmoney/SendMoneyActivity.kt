...
class SendMoneyActivity : BaseActivity() {
    ...
    private val publish by lazy { PublishSubject.create<SendMoneyNewAdapter.Interaction>() }
    ...
    private fun setAdapter() {
        rv_accounts_transfer.apply {
            ...
            adapter = SendMoneyNewAdapter(this@SendMoneyActivity, generateUsersTransferList(), publish)
        }
    }

    private fun itemClicked() {
        publish.ofType(SendMoneyNewAdapter.Interaction.ItemTapped::class.java).subscribe {
            showDialogToSendMoney(it.account)
        }
    }

    private fun imageClicked() {
        publish.ofType(SendMoneyNewAdapter.Interaction.ImageTapped::class.java).subscribe {
            showDialogToSendMoney(it.account)
        }
    }
